#!/usr/bin/env bash
# ==============================================================================
# curl.sh — Example: poll URL until expected HTTP status is received
# ==============================================================================
...
usage() {
    cat <<EOF
Usage: ${PROGNAME} [OPTIONS] HOST STATUS TIMEOUT

  Poll HOST until the HTTP response code matches STATUS, or until TIMEOUT
...
main() {
    ...
    [ -z "$HOST" ]    && fatal "HOST argument is required"
    [ -z "$STATUS" ]  && fatal "STATUS argument is required"
    [ -z "$TIMEOUT" ] && fatal "TIMEOUT argument is required"
