...
choice() {
	if [ "$CHOICE" == 1 ]; then
		CARD="$CARD_1"
		PROF="$CARD_1_PROFILE_1" # LG ULTRAWIDE
	elif [ "$CHOICE" == 2 ]; then
		CARD="$CARD_1"
		PROF="$CARD_1_PROFILE_2" # LG TV
	elif [ "$CHOICE" == 3 ]; then
		CARD="$CARD_0"
		PROF="$CARD_0_PROFILE_1" # Digital Output
	elif [ "$CHOICE" == 4 ]; then
		CARD="$CARD_0"
		PROF="$CARD_0_PROFILE_2" # Headphones
	else
		...
		read CHOICE
		echo
		choice # call the function again
	fi
}
